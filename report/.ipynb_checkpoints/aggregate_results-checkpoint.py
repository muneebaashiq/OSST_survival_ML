...
def aggregate_results(config, results):
        ...
        mean_results = results.groupby(["Scaler", "Selector", "Bucketizer"]).mean().drop('Seed', axis=1).reset_index()
        std_results = results.groupby(["Scaler", "Selector", "Bucketizer"]).std().drop('Seed', axis=1).reset_index()

        aggregated_results = mean_results.merge(
            std_results, on=["Scaler", "Selector", "Bucketizer"], suffixes=('_mean', '_std')
        )
